...
if(!empty($PROP)):
    ...
    $arCurrentValues['IBLOCK_ID'] = CATALOG_IBLOCK_ID;
    $offers = false;
    ...
    $arParamsFilter = array(
        ...
        'JS_FILE' => '/local/php_interface/include/script.js',
        'JS_MESSAGES' => Json::encode(array(
            'invalid' => 'Условие задано неверно'
        )),
    );
    ////
    
// p($filterDataValues, 'filterDataValues');
// p($arParamsFilter, 'arParamsFilter');
 //p($PROP, 'PROP');
	foreach($PROP as $prop_code=>$prop_fields):
		...
		?>
            <?
            //if($prop_code == 'CUSTOM_FILTER'){
            if($prop_fields["CODE"] == 'CUSTOM_FILTER'){
                if(count($prop_values) > 0){
                    foreach($prop_values as $val)
                        $value = $val;
                }
                else
                    $value = '';
    ...
    //p($value, 'value');
                //$tabControl->BeginCustomField('PROPERTY_CUSTOM_FILTER', 'Настройки отбора', $prop_fields["IS_REQUIRED"]==="Y");
                /*?>
                    <div id="custom_filter"></div>
                <?*/
                //$tabControl->EndCustomField('PROPERTY_CUSTOM_FILTER', '');
                ?>
                <tr>
                    <td class="adm-detail-valign-top" width="40%">
                        Настройки отбора:
                    </td>
                    <td width="60%" id="custom_filter">
                        <script>
                            //value = '<?=$value?>'
                            function DisplayParamCustom(param, value, container)
                            {
                                
                                param._propId = BX.util.htmlspecialchars(param.ID || Math.round(Math.random() * 10000)) + '_' + '123';
                            ...
                            var param = <?=CUtil::PhpToJSObject($arParamsFilter)?>;
                            container = BX('custom_filter');
                            DisplayParamCustom(param, '<?=$value?>', container);
